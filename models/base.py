...
def setup_cell(cell_type, size, use_residual=False, 
  ...
  cell = getattr(tf.contrib.rnn, cell_type)(size, activation=activation)
  if use_residual:
...
class ModelBase(object):
  def __init__(self, sess, config, trainer, vocab, activation=tf.nn.relu):
    self.sess = sess
    self.scope = tf.get_variable_scope()
    self.scopename = self.scope.name.replace('_', '/')

    # A hyperparameter for MTL to balance the loss of each task.
    ...
    self.lex_keep_prob = trainer.lex_keep_prob
