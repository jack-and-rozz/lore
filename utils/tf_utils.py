...
def flatten_timeseries(t):
  '''
  <Args>
  - t: An input tensor with shape [batch_size, time_step, :].
  <Return>
  - reshaped_t: The input tensor t flattened into [batch_size*time_step, :]
  - original_shape: Namely the original shape of t.
  '''
  with tf.name_scope(sys._getframe().f_code.co_name):
    original_shape = [shape(t, i) for i in range(len(t.get_shape()))]
    batch_size = original_shape[0]
    max_timestep = original_shape[1]
    reshaped_t = tf.reshape(t, [batch_size*max_timestep] + original_shape[2:])
  return reshaped_t, original_shape
...
class SharedKernelDense(tf.layers.Dense):
  def __init__(self, *args, shared_kernel=None, **kwargs):
    super(SharedKernelDense, self).__init__(*args, **kwargs)
    self.kernel = None
