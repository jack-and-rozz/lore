...
class MultiGPUTrainer(TrainerBase):
  ...
  def run_epoch(self, batches, is_training):
    ...
    while True:
      ...
      try:
        ...
        print('step', self.global_step.eval())
        for k, v in flatten_recdict(input_feed).items():
          if type(v) == np.ndarray:
            print(k.name, v.shape, np.max(v))
        print(e)
        print
        exit(1)
        #exit(1)
  ...
  def get_updates(self, tasks):
    with tf.name_scope("update"):
      assert len(list(tasks.keys())) == 1 # Assume single-task setting 
      task = list(tasks.keys())[0]
      params = tf.contrib.framework.get_trainable_variables()
      gradients = self.tasks[task].gradients 
